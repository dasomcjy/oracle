...
/* VIEW : 가상의 테이블, 데이터가 저장되어있지 않다. 
        ...
        -- 목적 : 1. 보안을 위해서 사용한느 경우 : 실제 테이블의 특정 컬럼을 숨겨서 처리
                2. 편의성을 위해서 사용하는 : 복잡한 구문을 View 로 만들어서 처리
                    JOIN 구문, 복잡한 구문, 
*/
...
/*
    뷰의 처리 과정
        ...
        1. user_views의 데이터 사전에서 뷰이름에 대한 쿼리를 조회
...
-- hiredate 컬럼의 제약 조건 위배 : NOT NULL
...
/*
    ...
    뷰 :
     ...
     2. 복합뷰 ( Complex View) : 두개 이상의 테이블로 생성된 view
         -- 뷰 내에서는 그룹함수 (SUM, AVG, MAX, MIN) , Group By 
            , ROWNUM 을 사용하지 못한다.
            -- 단 그룹함수를 사용할 경우 반드시 별칭이름을 사용하면 사용할 수 있다.
          
*/

-- ROWNUM : 레코드의 숫자를 출력해줌. ROW 의 넘버를 출력
    -- MS-SQL, MYSQL : TOP n 
...
--view 내부에 rownum 컬럼때문에 오류 발생(별칭 없는 경우)
...
-- View 내부에 Distinct는 별칭이름 없이 잘 생성됨. <Oracle 21c버젼에서 잘 생성됨>
...
-- View에서 그룹 함수 사용시 별칭이름을 부여해서 사용 : 오류가 발생되지 않는다.
...
-- view 내부에서 group by 를 사용할 경우 <<== 별칭이름을 사용하지 않아서 오류발생
   -- 별칭이름 사용 : 오류발생되지 않음
-- 각 부서별로 월급의 합계, 평균, 최대값, 최소값, count(*) 그룹핑수 
 create or replace view v_emp33
 as
 select dno, sum(salary) 부서별합계, round(AVG(salary),2) 부서별평균 , 
        max (Salary) 부서별최대값, min(Salary) 부서별최소값, count(*) 그룹된갯수
 from emp100
...
select * from v_emp33
 ...
 /*
   뷰를 생성시 옵션 사용
        ...
        -- noforce 옵션 : 반드시 기존테이블이 존재해야만 생성 << 기본값 >>
 */
 
 create or replace view v_emp34 
 as
 select * from emp101  ;  -- emp101번 테이블은 존재하지 않으므로 view생성시 오류 발생
...
/*  index 는 생성후 지속적인 관리가 필요함.
    ...
    -- 주기적으로 rebuild 해줘야한다. ( index 페이지를 업데이트 해야한다.)    
*/
...
/* ERD(Entity Relationship Diagram) : 데이터 베이스 모델링, 설계도
   ...
   1. 요구 사항 분석 : 업무 파악 , 고객의 협의 , 프로젝트의 범위를 지정(비용, 기간). <=== 
            테이블, 각 테이블의 속성 , 테이블 테이블의 관계  
